...
#include "NDPluginFile_Batch.h"

#include <epicsExport.h>

static const char *driverName="NDPluginFileBatch";
...
asynStatus NDPluginFileBatch::openFileBase(NDFileOpenMode_t openMode, NDArray *pArray)
{
...
asynStatus NDPluginFileBatch::closeFileBase()
{
    /* Closes a file */
    asynStatus status = asynSuccess;
    char fullFileName[2*MAX_FILENAME_LEN];
...
asynStatus NDPluginFileBatch::readFileBase(void)
{
    asynStatus status = asynSuccess;
    char fullFileName[2*MAX_FILENAME_LEN];
    NDArray *pArray=NULL;
...
asynStatus NDPluginFileBatch::writeFileBase()
{
...
void NDPluginFileBatch::freeCaptureBuffer()
{
    NDArray *pArray;
...
asynStatus NDPluginFileBatch::doCapture(int capture)
{
 ...
 */
asynStatus NDPluginFileBatch::attrFileCloseCheck()
{
    asynStatus status = asynSuccess;
 ...
 */
asynStatus NDPluginFileBatch::attrFileNameCheck()
{
    asynStatus status = asynSuccess;
 ...
 */
asynStatus NDPluginFileBatch::attrFileNameSet()
{
    asynStatus status = asynSuccess;
    NDAttribute *ndAttr;
    char attrFileName[MAX_FILENAME_LEN];
 ...
 */
bool NDPluginFileBatch::attrIsProcessingRequired(NDAttributeList* pAttrList)
{
...
void NDPluginFileBatch::registerInitFrameInfo(NDArray *pArray)
{
...
bool NDPluginFileBatch::isFrameValid(NDArray *pArray)
{
    if (pArray == NULL) {
        asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
                "NDPluginFileBatch::isFrameValid: pArray == NULL\n");
        return false;
    }
    NDArrayInfo_t info;
    ...
    if (initInfo == NULL) {
        asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
                "NDPluginFileBatch::isFrameValid: no init array info was registered\n");
        return false;
    }
    bool valid = true;
    ...
    if (pArray->getInfo(&info) != ND_SUCCESS) {
        asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
                "NDPluginFileBatch::isFrameValid: Unable to get NDArray info from pArray=%p\n", pArray);
        return false;
    }
    ...
    if (initInfo->bytesPerElement != info.bytesPerElement) {
        asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
                "NDPluginFileBatch::isFrameValid: WARNING: Datatype bit-width has changed. Was: %dbytes now: %dbytes\n",
    ...
    if ((initInfo->xSize != info.xSize) || (initInfo->ySize != info.ySize)) {
        asynPrint(this->pasynUserSelf, ASYN_TRACE_ERROR,
                "NDPluginFileBatch::isFrameValid: WARNING: Frame dimensions have changed X:%lu,%lu Y:%lu,%lu]\n",
...
/** Callback function that is called by the NDArray driver with new NDArray data.
  ...
  */
void NDPluginFileBatch::processCallbacks(NDArray *pArray)
{
	...
	if (fileWriteMode == NDFileModeCapture || fileWriteMode == NDFileModeStream)
	{
		if (autoSave && !capture)
		{
			capture = 1;
...
asynStatus NDPluginFileBatch::writeInt32(asynUser *pasynUser, epicsInt32 value)
{
...
asynStatus NDPluginFileBatch::writeNDArray(asynUser *pasynUser, void *genericPointer)
{
...
/** Constructor for NDPluginFileBatch; all parameters are simply passed to NDPluginDriver::NDPluginDriver.
  ...
  */
NDPluginFileBatch::NDPluginFileBatch(const char *portName, int queueSize, int blockingCallbacks,
...
{
    //static const char *functionName = "NDPluginFileBatch";
    ...
    setStringParam(NDPluginDriverPluginType, "NDPluginFileBatch");
